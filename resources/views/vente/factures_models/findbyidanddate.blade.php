...
<head>
    <title>izyStock | Point du {{$_GET['date']}} </title>
    <!-- Custom CSS -->
...
<body>
	<div class="container">
        ...
        @foreach ($lesventes as $item)
            @if ($item->created_at->format('d/m/yy') == $_GET['date'] || $item->created_at->format('Y-m-d') == $_GET['date'] )
        ...
        <div class="col-12">
			...
			<span style="float: right;"><b>Reccette de la journée</b> : {{ $total }} FCFA </span>
        </div>
        <br>

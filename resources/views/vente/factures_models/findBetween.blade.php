...
<head>
    <title>izyStock | Inventaire personnalisée entre {{$_GET['date1']}} et {{$_GET['date2']}}</title>
    <!-- Custom CSS -->
...
<body>
	<div class="container">
		...
		<div class="text-center col-12">
			...
			<div style="float: right; font-size: 15px;"> <b>Inventaire personnalisée entre {{$_GET['date1']}} et {{$_GET['date2']}} </b> </div><br>
        </div>
		<hr>
        <br>
        <?php 
            if ($_GET['user'] == 0) {
                ...
                # code...
                $lesventes = App\Facture::where('idemploye', $_GET['user'])
                                            // ->where( 'created_at', '>', date("d/m/yy"))
                                            ->whereBetween('created_at', [$_GET['date1'], $_GET['date2']])
                                            ->get();
            }
        ...
        @endforeach
		<div class="col-12">
			<span><b>Vendeur</b> : {{ $_GET['user'] == 0 ? 'Tous les utilisateurs' : App\User::findOrFail( $_GET['user'])->name}} </span>
			<span style="float: right;"><b>Date</b> : [ {{ $_GET['date1'] }} ; {{ $_GET['date2'] }} ] </span>
        </div>
        <br>
        <div class="col-12">
			<span><b>Nombre de vente</b> : {{ $nbrevente}} </span>
			<span style="float: right;"><b>Reccette de la période<0/b> : {{ $total }} FCFA </span>
        </div>
        <br>
        ...
        @php
            if ($_GET['user'] == 0) {
                # code...
            $toutesventes = App\Vente::where('idannexe', AUth()->User()->idannexe)
                                    ->whereBetween('created_at', [$_GET['date1'], $_GET['date2']])
                                    ->get();
                ...
                # code...
            $toutesventes = App\Vente::where('idemploye', $_GET['user'])
                                        ->whereBetween('created_at', [$_GET['date1'], $_GET['date2']])
                                        ->get();
        ...
        <div>
            <table style="border-collapse: collapse;">
                ...
                <tbody>
                    ...
                    @endforeach
                    <tr>
                        <td  {!! $_GET['user'] == 0 ? 'colspan="6"' : 'colspan="6"' !!}><b>TOTAL : </b></td>
                        <td style="border: 1px solid black; padding: 10px; " {!! $_GET['user'] == 0 ? 'colspan="2"' : '' !!}><b>{{ $total }}</b></td>
        ...
        <br>
        @if ($_GET['user'] == 0)
        <div><center> <b>Par utilisateur</b> </center></div>
        <br>
        ...
        <div>
            <table style="border-collapse: collapse;">
                ...
                <tbody>
                    @php
                        $toususers = App\User::where('idannexe', AUth()->User()->idannexe)->get();
                    @endphp
                    @foreach ($toususers as $ite)
                        <tr>
                            <th colspan="7" style="border: 1px solid black; padding: 10px;"><center>{{ $ite->name }}</center></th>
                        </tr>
                        @php                                
                            $toutesventes = App\Vente::where('idemploye', $ite->id)->get();
                            $count = 0;
                        @endphp
                        ...
                        @if ($count == 0)
                        <tr>
                            <td colspan="7" style="border: 1px solid black; padding: 10px;"><center>Aucune vente pour cet utilisateur</center></td>
                        </tr>
                        @endif
                    @endforeach
                    <tr>
